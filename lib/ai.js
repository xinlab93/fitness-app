...
import { loadHealth, buildHealthContext } from './storage.js';
...
export async function callAI(userMessage, options = {}) {
  setupProxy();

  let finalUserMessage = userMessage;
  if (options.includeHealth !== false) {
    const health = await loadHealth();
    const ctx = buildHealthContext(health);
    if (ctx) finalUserMessage = userMessage + ctx;
  }
